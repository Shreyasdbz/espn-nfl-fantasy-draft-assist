...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      {view === 'players' && <section className="full-page players-page"><div className="page-title"><div><span className="eyebrow">PLAYER INTELLIGENCE</span><h1>Available player pool</h1><p>{catalogCopy.description}</p></div><Badge variant="outline" className="revision-stamp">{catalogCopy.badge}</Badge></div><div className="filter-bar"><label className="search-box"><Search /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search players" aria-label="Search players" /></label><ToggleGroup type="single" value={position} onValueChange={(value) => { if (positions.includes(value as typeof position)) setPosition(value as typeof position); }} className="position-tabs" aria-label="Player position filter">{positions.map((item) => <ToggleGroupItem key={item} value={item} aria-label={`Show ${item} players`}>{item}</ToggleGroupItem>)}</ToggleGroup><label className="toggle"><Switch checked={hideDrafted} onCheckedChange={setHideDrafted} />Available only</label></div><PlayerTable players={filteredPlayers} onDetail={setDetail} /></section>}
