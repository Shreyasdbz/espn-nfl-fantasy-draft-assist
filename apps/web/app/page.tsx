...
function isFresh(value: string | null, referenceTime: number, maxAgeMs = 15_000) {
  if (!value) return false;
  const timestamp = new Date(value).getTime();
  return Number.isFinite(timestamp) && referenceTime - timestamp <= maxAgeMs && timestamp - referenceTime <= 5_000;
}
...
export default function Home() {
  ...
  useEffect(() => {
    const refreshClock = () => setReferenceTime(Date.now());
  ...
  useEffect(() => {
    const focusPlayerSearch = (event: KeyboardEvent) => {
    ...
    window.addEventListener('keydown', focusPlayerSearch);
    return () => window.removeEventListener('keydown', focusPlayerSearch);
  }, []);
  ...
  const observationFresh = isFresh(health.lastObservationAt, referenceTime);
  const reconciliationFresh = state.picks.length === 0 || isFresh(health.lastReconciledAt, referenceTime);
  const synchronized = health.pageAttached && health.capture === 'healthy' && observationFresh && reconciliationFresh;
  const synchronizationLabel = !health.pageAttached ? 'Offline' : synchronized ? 'Synced' : 'Stale';
  const decisionBlocked = state.recommendationContext.picksRemaining > 0 && !synchronized;
  ...
  const recommendationIds = new Set(state.recommendations.slice(0, 6).map((recommendation) => recommendation.playerId));
  const livePool = filteredPlayers.filter((player) => !player.drafted && !recommendationIds.has(player.id)
    && (state.recommendationContext.gate !== 'forced-needs' || state.recommendationContext.requiredPositions.includes(player.position))).slice(0, 9);
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <ToggleGroup type="single" value={view} onValueChange={(value) => { if (value === 'live' || value === 'players') setView(value); }} className="primary-nav" aria-label="Workspace view"><ToggleGroupItem value="live" aria-label="Live view"><Radio />Live</ToggleGroupItem><ToggleGroupItem value="players" aria-label="Players view"><Users />Players</ToggleGroupItem></ToggleGroup>
        <div className="header-actions">
          <Popover><PopoverTrigger asChild><Button variant="ghost" size="sm" className="sync-button"><span className={`status-dot ${synchronized ? 'ok' : ''}`} />{synchronizationLabel}<ChevronDown /></Button></PopoverTrigger><PopoverContent align="end" className="sync-popover"><div><strong>ESPN watcher</strong><Badge variant="outline">READ ONLY</Badge></div><p>{health.pageAttached ? synchronized ? `Last board update ${health.lastReconciledAt ? new Date(health.lastReconciledAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', second: '2-digit' }) : 'just now'}` : 'The ESPN page is attached, but the latest observation or reconciliation is stale.' : 'No ESPN draft is currently attached.'}</p><div className="sync-grid"><StatusPip label="Engine" ok={health.engine === 'healthy'} detail={`Engine ${health.engine}`} /><StatusPip label="Capture" ok={health.capture === 'healthy' && observationFresh} detail={observationFresh ? `Capture ${health.capture}` : 'No recent ESPN observation'} /><StatusPip label="Draft" ok={health.pageDetected && reconciliationFresh} detail={!health.pageDetected ? 'No draft page' : reconciliationFresh ? 'Draft page detected and reconciled' : 'Draft reconciliation is stale'} /></div><Button variant="outline" size="sm" onClick={() => setWatcherOpen(true)}>Connection details</Button>{health.pageDetected && !health.pageAttached && <Button size="sm" onClick={() => void mutate('bind', '/v1/browser/bind-page', { commandId: commandId('bind-page'), expectedRevision: state.session.revision, body: {} }, 'POST', 'Following this ESPN draft read-only.')}>Follow draft</Button>}</PopoverContent></Popover>
      ...
      {view === 'live' && <section className="live-page">
        ...
        {decisionBlocked && <div className="freshness-warning" role="alert"><Activity /><div><strong>{health.pageAttached ? 'Recommendations paused: ESPN data is stale' : 'Recommendations paused: ESPN is offline'}</strong><span>Confirm the draft watcher reconnects before acting. The board below is preserved for reference.</span></div></div>}
        <ConstructionStrip state={state} />
        <div className={`live-grid ${decisionBlocked ? 'decision-stale' : ''}`}><div className="decision-column"><div className="compact-heading"><div><span className="eyebrow">{state.recommendationContext.picksRemaining ? 'PICK NOW' : 'DRAFT COMPLETE'}</span><h2>{state.recommendationContext.picksRemaining ? 'Best fits for this roster' : 'Roster construction complete'}</h2></div><span>{state.recommendationContext.picksRemaining ? synchronized ? 'updates after every pick' : 'waiting for fresh ESPN data' : `${state.roster.length} selections captured`}</span></div>{state.recommendations.length ? <div className="recommendation-list" aria-label="Ranked player recommendations"><div className="recommendation-column-heads" aria-hidden="true"><span>RK</span><span>Pos</span><span>Player</span><span>Fit</span><span>Availability</span><span>Why</span></div>{state.recommendations.slice(0, 6).map((recommendation) => <RecommendationRow key={recommendation.playerId} recommendation={recommendation} player={state.players.find((player) => player.id === recommendation.playerId)} open={factorsOpen === recommendation.playerId} onOpen={() => setFactorsOpen(factorsOpen === recommendation.playerId ? null : recommendation.playerId)} onInspect={() => setDetail(state.players.find((player) => player.id === recommendation.playerId) ?? null)} />)}</div> : state.recommendationContext.picksRemaining === 0 ? <Card className="draft-complete"><CircleCheckBig /><div><strong>No more picks required</strong><p>The watcher captured a full roster. Review the remaining pool or open Draft board from the tools menu.</p></div></Card> : <Card className="draft-complete draft-blocked"><Activity /><div><strong>No legal recommendation is available</strong><p>{state.recommendationContext.requiredPositions.length ? `The player catalog has no available ${state.recommendationContext.requiredPositions.join('/')} candidate.` : 'The user has no remaining draft turn, or the captured roster/configuration is incomplete.'} Reconcile ESPN before acting.</p></div></Card>}<div className="compact-heading board-heading"><div><span className="eyebrow">NEXT TIER</span><h2>{state.recommendationContext.gate === 'forced-needs' ? `Available ${state.recommendationContext.requiredPositions.join(' / ')}` : 'Available players'}</h2></div><Button variant="ghost" size="sm" onClick={() => setView('players')}>All players →</Button></div><PlayerTable players={livePool} onDetail={setDetail} compact label="Next-tier available players" /></div>
      ...
      {view === 'players' && <section className="full-page players-page"><div className="page-title"><div><span className="eyebrow">PLAYER INTELLIGENCE</span><h1>Available player pool</h1><p>{catalogCopy.description}</p></div><Badge variant="outline" className="revision-stamp">{catalogCopy.badge}</Badge></div><div className="filter-bar"><label className="search-box"><Search /><Input ref={searchInputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search players" aria-label="Search players" aria-keyshortcuts="/" /><Kbd aria-hidden="true">/</Kbd></label><ToggleGroup type="single" value={position} onValueChange={(value) => { if (positions.includes(value as typeof position)) setPosition(value as typeof position); }} className="position-tabs" aria-label="Player position filter">{positions.map((item) => <ToggleGroupItem key={item} value={item} aria-label={`Show ${item} players`}>{item}</ToggleGroupItem>)}</ToggleGroup><label className="toggle"><Switch checked={hideDrafted} onCheckedChange={setHideDrafted} />Available only</label></div><PlayerTable players={filteredPlayers} onDetail={setDetail} label="Available player pool" /></section>}
      ...
      <Sheet open={!!detail} onOpenChange={(open) => { if (!open) setDetail(null); }}><SheetContent className="player-drawer">{detail && <PlayerDrawer player={detail} recommendation={state.recommendations.find((item) => item.playerId === detail.id)} copyDisabled={decisionBlocked} onCopy={async () => { await navigator.clipboard.writeText(detail.name); toast.success(`${detail.name} copied. Make the selection on ESPN.`); }} />}</SheetContent></Sheet>
      <Dialog open={watcherOpen} onOpenChange={setWatcherOpen}><DialogContent className="modal"><DialogHeader><span className="eyebrow">READ-ONLY ESPN CONNECTION</span><DialogTitle>Fourth Down ESPN Watcher</DialogTitle><DialogDescription>The Chrome extension observes rendered draft picks and relays them to this local engine. It cannot queue players or make selections.</DialogDescription></DialogHeader><dl><div><dt>Status</dt><dd><Badge variant="outline" className={synchronized ? 'drafted-label' : ''}>{!health.pageAttached ? 'NOT DETECTED' : synchronized ? 'SYNCHRONIZED' : 'STALE'}</Badge></dd></div><div><dt>Capture</dt><dd>{health.capture}</dd></div><div><dt>Last snapshot</dt><dd>{health.lastObservationAt ? new Date(health.lastObservationAt).toLocaleTimeString() : 'None'}</dd></div><div><dt>Extension folder</dt><dd><code>apps/chrome-extension</code></dd></div></dl><DialogDescription>Install once from <code>chrome://extensions</code> using Developer mode → Load unpacked. The legacy bridge remains available only as a diagnostic fallback.</DialogDescription><DialogFooter className="modal-actions"><Button variant="outline" className="secondary-button" onClick={() => void copyTabBridge()} disabled={busy === 'bridge'}>Copy fallback bridge</Button><Button onClick={() => setWatcherOpen(false)}>Done</Button></DialogFooter></DialogContent></Dialog>
